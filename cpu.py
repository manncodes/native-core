...
regnames = (
    (
        (
            (
                (["x0", "ra", "sp", "gp", "tp"] + ["t%d" % i for i in range(3)])
                + ["s0", "s1"]
            )
...
def step():
    ...
    alt = (funct7 == 0b0100000) and (
        opcode == Ops.OP or (opcode == Ops.IMM and funct3 == Funct3.SRAI)
    )
    imm = {
        Ops.JAL: imm_j,
        Ops.JALR: imm_i,
    ...
    arith_left = (
        vpc
        if opcode in [Ops.JAL, Ops.BRANCH, Ops.AUIPC]
        else (0 if opcode == Ops.LUI else vs1)
    )
